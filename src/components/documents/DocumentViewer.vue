...
import { Trash2, Brain, FileSearch, UserCheck, ListChecks, GraduationCap } from 'lucide-vue-next';
...
<template>
  ...
  <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 h-full overflow-y-hidden animate-fade-in">
    ...
    <div class="bg-slate-900 rounded-xl border border-slate-700 overflow-y-auto custom-scrollbar p-4 flex flex-col">
      <h3 class="text-sm font-bold text-slate-400 mb-4 flex items-center sticky top-0 bg-slate-900 py-2 z-10 shrink-0">
        <FileSearch class="w-4 h-4 mr-2" /> Documento Original
      </h3>
